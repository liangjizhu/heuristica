...
This repository contains Python scripts for solving **constraint satisfaction problems (CSP)** and **pathfinding** problems. The scripts are organized into different directories based on their functionality and purpose.
...
To set up the environment and install the dependencies:
...
### Running constraint satisfaction (CSP) scripts

To run the constraint satisfaction problem scripts:
...
### Running the maintenance scheduling script

To run the maintenance scheduling script:

```bash
python parte-1/CSPMaintenance.py <ruta_fichero_entrada>
```

### Running the pathfinding script

To run the pathfinding script:
...
- Constraint satisfaction (CSP) tests:
